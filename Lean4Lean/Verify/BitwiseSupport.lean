...
/-- The constructor fragment of `HasPrimitives` used by the bitwise
certificate proofs.  Unlike the full primitive invariant, this fragment is
monotone under arbitrary environment extensions, which is required by the
Kripke quantifier in `ReflectsNatBitwise`. -/
...
theorem VEnv.HasNatBoolConstructors.of_primitives {env : VEnv}
    ...
    Lean4Lean.Environment.VEnv.HasNatBoolConstructors env := by
...
theorem VEnv.HasNatBoolConstructors.mono {env env' : VEnv}
    (h : Lean4Lean.Environment.VEnv.HasNatBoolConstructors env)
    (le : env ≤ env') :
    Lean4Lean.Environment.VEnv.HasNatBoolConstructors env' where
...
theorem VEnv.HasNatBoolConstructors.boolFalseS
  ...
  exact ⟨.const h.boolFalse rfl rfl, .const h.boolFalse nofun rfl⟩
...
theorem VEnv.HasNatBoolConstructors.boolTrueS
  ...
  exact ⟨.const h.boolTrue rfl rfl, .const h.boolTrue nofun rfl⟩

theorem VEnv.HasNatBoolConstructors.boolLitS
    (h : Lean4Lean.Environment.VEnv.HasNatBoolConstructors env) (b : Bool) :
...
theorem VEnv.HasNatBoolConstructors.natZeroS
    ...
    TrExprS env Us Δ .natZero .natZero ∧
      env.HasType Us.length Δ.toCtx .natZero .nat := by
  exact ⟨.const h.natZero rfl rfl, .const h.natZero nofun rfl⟩

theorem VEnv.HasNatBoolConstructors.natSuccS
    (h : Lean4Lean.Environment.VEnv.HasNatBoolConstructors env) :
    TrExprS env Us Δ .natSucc .natSucc ∧
      env.HasType Us.length Δ.toCtx .natSucc
        (.forallE .nat .nat) := by
  exact ⟨.const h.natSucc rfl rfl, .const h.natSucc nofun rfl⟩

theorem VEnv.HasNatBoolConstructors.natLitS
    (h : Lean4Lean.Environment.VEnv.HasNatBoolConstructors env) (n : Nat) :
