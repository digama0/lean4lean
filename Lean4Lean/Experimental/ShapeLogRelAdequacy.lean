...
theorem LR.toValTy (le_n : n ≤ n') (le_a : b.lift (n := n) (m := n') ≤ m)
...
theorem forallE_whRed_l (d : Γ ⊢ A₀ ≡ SExpr.forallE B₁ F₁ : .sort s) :
  ...
  have := (LR.adequacy d ((LE_Interp.sound d .nil).1.2 hPi) (.sort .rfl) hmem).2 .id
